...
pub fn size_bytes(val: &Bson) -> usize {
    match val {
        ...
        //   * number of UTF-8 bytes
        //   * null terminator (1 byte)
        Bson::Regex(ref regex) => regex.pattern.len() + 1 + regex.options.len() + 1,
        //
        // * length prefix (4 bytes)
        // * number of UTF-8 bytes
        // * null terminator (1 byte)
        ...
        // * length of document
        Bson::JavaScriptCodeWithScope(ref source) => {
            4 + 4 + source.code.len() + 1 + doc_size_bytes(&source.scope)
        }
        Bson::I32(_) => 4,
        Bson::I64(_) => 8,
        Bson::TimeStamp(_) => 8,
        ...
        // * subtype (1 byte)
        // * number of bytes
        Bson::Binary(ref binary) => 4 + 1 + binary.bytes.len(),
        Bson::ObjectId(_) => 12,
...
mod test {
    ...
    use crate::bson::{self, oid::ObjectId, spec::BinarySubtype, Bson, Binary, Regex, TimeStamp, JavaScriptCodeWithScope};
    use crate::doc;
    ...
    fn doc_size_bytes_eq_serialized_size_bytes() {
        let doc = doc! {
            "double": -12.3,
            "string": "foo",
            "array": ["foobar", -7, Bson::Null, Bson::TimeStamp(TimeStamp::new(1278)), false],
            "document": {
                "x": 1,
            ...
            "bool": true,
            "null": Bson::Null,
            "regex": Bson::Regex(Regex::new("foobar".into(), "i".into())),
            "code": Bson::JavaScriptCode("foo(x) { return x + 1; }".into()),
            "code with scope": Bson::JavaScriptCodeWithScope(
                JavaScriptCodeWithScope::new("foo(x) { return x + y; }".into(), doc! { "y": -17 })
            ),
            "i32": 12i32,
            "i64": -126i64,
            "timestamp": Bson::TimeStamp(TimeStamp::new(1223334444)),
            "binary": Bson::Binary(Binary::new(BinarySubtype::Generic, vec![3, 222, 11])),
