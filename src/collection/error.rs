...
impl WriteException {
    ...
    pub fn validate_write_result(result: &Document, write_concern: WriteConcern) -> Result<()> {
...
impl BulkWriteException {
    ...
    pub fn validate_bulk_write_result(
        ...
        let w_errs = if let Some(&Bson::Array(ref errors)) = result.get("writeErrors") {
            if errors.is_empty() {
                return Err(Error::ResponseError("Server indicates a write error, \
                                                              but none were found.".to_string()));
            }
