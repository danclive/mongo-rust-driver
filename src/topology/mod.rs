...
impl TopologyDescription {
    ...
    fn get_nearest_from_vec(&self, servers: &mut Vec<Host>) -> Result<(PooledStream, ServerType)> {
        ...
        Err(OperationError("No servers available for the provided ReadPreference.".to_string()))
    }
    ...
    pub fn acquire_write_stream(&self) -> Result<PooledStream> {
        ...
        if rand {
            Ok(self.get_rand_from_vec(&mut hosts)?.0)
        } else {
            Ok(self.get_nearest_from_vec(&mut hosts)?.0)
        }
    }
    ...
    pub fn choose_write_hosts(&self) -> (Vec<Host>, bool) {
        ...
        match self.topology_type {
            ...
            _ => {
                (self.servers.keys().filter_map(|host| {
                    if let Some(server) = self.servers.get(host) {
                        if let Ok(description) = server.description.read() {
    ...
    fn update_rs_from_primary(
        ...
        let description_set_name = description.read().unwrap().set_name.to_string();
        ...
        let valid_hosts: Vec<_> = {
            ...
            description_guard.hosts.iter().cloned()
    ...
    fn update_rs_without_primary(
        ...
        let set_name = description.read().unwrap().set_name.to_string();
    ...
    pub fn new(
        ...
        if let Some(ref config_opts) = config.options {
            if let Some(name) = config_opts.options.get("replicaSet") {
                options.set_name = name.to_string();
        ...
        if !options.set_name.is_empty() &&
           options.topology_type != TopologyType::ReplicaSetNoPrimary {
            return Err(ArgumentError("TopologyType must be ReplicaSetNoPrimary if \
                                                   set_name is provided.".to_string()));
        }
