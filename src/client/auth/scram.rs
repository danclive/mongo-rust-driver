...
use crate::util::md5;
use crate::doc;
use crate::bson::{spec::BinarySubtype, Bson, Document, Binary};
...
impl ScramVersion {
    ...
    pub(crate) fn authenticate_stream(
        ...
        let noop = doc! {
            ...
            "payload": Bson::Binary(Binary::new(BinarySubtype::Generic, Vec::new()))
        };
    ...
    fn compute_salted_password(
        ...
        let normalized_password = match self {
            ScramVersion::Sha1 => {
                ...
                let md5 = md5::compute(format!("{}:mongo:{}", username, password));
                Cow::Owned(format!("{:x}", md5))
            }
...
impl ClientFirst {
    ...
    fn to_command(&self, scram: &ScramVersion) -> Document {
        doc! {
            ...
            "payload": Bson::Binary(Binary::new(BinarySubtype::Generic, self.message().as_bytes().to_vec()))
        }
    }
}
...
impl ClientFinal {
    ...
    fn payload(&self) -> Bson {
        Bson::Binary(Binary::new(BinarySubtype::Generic, self.message().as_bytes().to_vec()))
    }
